...
The `admin/admin` credentials above belong only to the local development Keycloak console. They are not a Catalogi account and cannot be used to administer Catalogi. Catalogi users authenticate through OIDC; initialize its first administrator with `CATALOGI_INITIAL_ADMIN_EMAIL` as described in [Authentication](3.1-authentication.md).
